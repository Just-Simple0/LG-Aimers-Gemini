"""
v20 후보 실험: Trackman Physics Features (물리적 궤적 및 회전 지표 정제)
========================================================================

배경 및 가설:
  - 현재 Trackman 3-key(55개 카운트 셀) 룩업에서는 원시 물리 평균값(rel_speed, zone_speed, spin_rate 등)만 매핑되어 있음.
  - 야구 세이버메트릭스 물리 지표:
    1) Bauer Units (구속 대비 회전 효율):
       - tk_bauer_units = tk_spin_rate_mean / tk_rel_speed_mean
    2) Speed Loss Ratio (플레이트 도달 감속률):
       - tk_speed_loss_ratio = (tk_rel_speed_mean - tk_zone_speed_mean) / tk_rel_speed_mean
    3) Movement Total Dispersion (카운트별 상하/좌우 무브먼트 합성 산포도):
       - tk_movement_dispersion = sqrt(tk_vert_break_std^2 + tk_horz_break_std^2)
    4) Release Point (릴리스 높이/사이드 평균):
       - tk_rel_height_mean, tk_rel_side_mean
  - 가설:
    리그 물리 상호작용 지표 4개를 추가하여 모델이 볼카운트별 투구 물리 특성(궤적 분산, 회전 효율, 도달 속도 감속)을
    더 정밀하게 학습함으로써 BSS를 개선할 수 있을 것임.
...
def make_tk_lookup_base(tk_df):
    """기존 v14 Trackman lookup."""
...
def make_tk_lookup_v20(tk_df):
    """v20 확장 Trackman lookup (물리 지표 정제)."""
    lookup = tk_df.groupby(TK_KEYS).agg(
        tk_fastball_rate=("pitch_type_group", lambda s: (s == "fastball").mean()),
        tk_breaking_rate=("pitch_type_group", lambda s: (s == "breaking").mean()),
        tk_offspeed_rate=("pitch_type_group", lambda s: (s == "offspeed").mean()),
        ...
        tk_vert_break_std=("induced_vert_break", "std"),
        tk_extension_mean=("extension", "mean"),
        tk_rel_height_mean=("rel_height", "mean"),
        tk_rel_side_mean=("rel_side", "mean"),
    ).reset_index()

    # 물리 파생 계산
    lookup["tk_bauer_units"] = lookup["tk_spin_rate_mean"] / lookup["tk_rel_speed_mean"]
    lookup["tk_speed_loss_ratio"] = (lookup["tk_rel_speed_mean"] - lookup["tk_zone_speed_mean"]) / lookup["tk_rel_speed_mean"]
    lookup["tk_movement_dispersion"] = np.hypot(lookup["tk_vert_break_std"], lookup["tk_horz_break_std"])
    return lookup


def build_features_v14(df, global_mean, tk_lookup):
    df = df.copy()
...
def main():
    print("=" * 70)
    print("v20 후보 실험: Trackman Physics Features (물리적 궤적 및 회전 지표 정제)")
    ...
    TK_LOOKUP_VAL_BASE = make_tk_lookup_base(tk_raw[tk_raw["season"] <= 2023])
    TK_LOOKUP_VAL_V20 = make_tk_lookup_v20(tk_raw[tk_raw["season"] <= 2023])

    # 3. Season Decomposition 공통 생성
    print("\nSeason Decomposition 생성 중...")
    ...
    # 4. v14 기준 피처 및 v20 확장 피처 결합
    print("피처 세트 구성 중...")
    base_fe_v14 = build_features_v14(train_full, GLOBAL_MEAN_VAL, TK_LOOKUP_VAL_BASE)
    fe_v14 = pd.concat([base_fe_v14, season_decomp_all], axis=1)

    base_fe_v20 = build_features_v14(train_full, GLOBAL_MEAN_VAL, TK_LOOKUP_VAL_V20)
    fe_v20 = pd.concat([base_fe_v20, season_decomp_all], axis=1)
    ...
    NEW_COLS_V20 = [c for c in fe_v20.columns if c not in train_full.columns]
    ALL_FEATURES_V20 = CAT_COLS + [c for c in FEATURES_BASE if c not in CAT_COLS] + NEW_COLS_V20

    for c in CAT_COLS:
        fe_v14[c] = fe_v14[c].astype("category")
        fe_v20[c] = fe_v20[c].astype("category")

    val_fe_v14 = fe_v14[fe_v14["season"] == 2024].copy()
    val_fe_v20 = fe_v20[fe_v20["season"] == 2024].copy()
    ...
    res_v20 = run_one(
        "WITH (v20 Trackman 물리 정제 5개 추가, 106개 피처)",
        fe_v20, val_fe_v20, ALL_FEATURES_V20,
    )
    results.append(res_v20)

    # 6. 최종 요약
    diff_A = res_v20["score_A_holdout"] - res_v14["score_A_holdout"]
    diff_B = res_v20["score_B_oof"] - res_v14["score_B_oof"]

    print("\n" + "=" * 70)
    print("최종 결과 요약 (Trackman Physics Features)")
    print("=" * 70)
    print(f"{'항목':<35} {'v14(WITHOUT)':>12} {'v20(WITH)':>12} {'diff':>10}")
    print(f"{'피처 수':<35} {res_v14['n_features']:>12} {res_v20['n_features']:>12}")
    print(f"{'LGB solo':<35} {res_v14['lgb_solo']:>12.2f} {res_v20['lgb_solo']:>12.2f}")
    print(f"{'CB solo':<35} {res_v14['cb_solo']:>12.2f} {res_v20['cb_solo']:>12.2f}")
    print(f"{'(A) 홀드아웃':<35} {res_v14['score_A_holdout']:>12.2f} {res_v20['score_A_holdout']:>12.2f} {diff_A:>+10.2f}")
    print(f"{'(B) OOF score':<35} {res_v14['score_B_oof']:>12.2f} {res_v20['score_B_oof']:>12.2f} {diff_B:>+10.2f}")
    ...
    out = "docs/v20_trackman_physics_results.csv"
