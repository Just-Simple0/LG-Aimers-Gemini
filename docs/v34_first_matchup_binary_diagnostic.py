"""
v34 후보 실험: First-Time Matchup Binary Indicator (초면 맞대결 단일 바이너리 피처)
=====================================================================================

배경 및 가설:
  - v33에서 맞대결 이력을 추가했을 때 LightGBM solo는 809.27 -> 827.34 (+18.07)로 급상승함.
  - 그러나 소표본 연속형 비율인 pair_history_posterior의 노이즈로 인해 CatBoost의 oblivious tree 분할 효율이 저하됨.
  - 가설:
    노이즈가 있는 연속형 사후확률을 배제하고,
    순수하게 '통산 첫 맞대결 여부(pair_is_first_matchup)' 1개의 바이너리 피처만 추가하면:
    1) CatBoost의 트리 깊이 예산을 낭비하지 않고
    2) LightGBM의 낯선 타자 상대 제구력 분별력을 보존하여
    3) 이중 게이트(A/B)와 5대 하드 가드레일을 동시 통과할 수 있을 것임.
...
class FirstMatchupEncoder:
    """초면 맞대결 여부 단일 바이너리 인코더."""
    ...
    def fit(self, df, target=None):
        ...
        self.past_pairs = set(pairs.unique())
        return self

    def transform(self, df):
        pairs = df["pitcher_id"].astype(str) + "_" + df["batter_id"].astype(str)
        is_first = (~pairs.isin(self.past_pairs)).astype(np.int8)
        return pd.DataFrame({"pair_is_first_matchup": is_first}, index=df.index)
...
def make_temporal_first_matchup(train_df):
    seasons = sorted(train_df["season"].unique())
    chunks = []
    for s in seasons:
        s_mask = train_df["season"] == s
        past_mask = train_df["season"] < s
        if past_mask.sum() == 0:
            encoder = FirstMatchupEncoder()
            dummy_df = pd.DataFrame({"pitcher_id": [-999], "batter_id": [-999]})
            encoder.fit(dummy_df)
            chunks.append(encoder.transform(train_df.loc[s_mask]))
        else:
            encoder = FirstMatchupEncoder().fit(train_df.loc[past_mask])
            chunks.append(encoder.transform(train_df.loc[s_mask]))
...
def main():
    print("=" * 70)
    print("v34 후보 실험: First-Time Matchup Binary Indicator")
    ...
    # 3. v34 초면 맞대결 바이너리 피처 생성 (1개 피처만 단독 추가)
    print("\nv34 초면 맞대결 바이너리 (pair_is_first_matchup) 피처 생성 중...")
    first_decomp_train = make_temporal_first_matchup(train_split)
    first_encoder_val = FirstMatchupEncoder().fit(train_split)
    first_decomp_val = first_encoder_val.transform(val_split)
    first_decomp_all = pd.concat([first_decomp_train, first_decomp_val], axis=0).loc[train_full.index]

    fe_v34 = pd.concat([fe_v14, first_decomp_all], axis=1)
    ...
    NEW_COLS_V34 = [c for c in fe_v34.columns if c not in train_full.columns]
    ALL_FEATURES_V34 = CAT_COLS + [c for c in FEATURES_BASE if c not in CAT_COLS] + NEW_COLS_V34

    for c in CAT_COLS:
        fe_v14[c] = fe_v14[c].astype("category")
        fe_v34[c] = fe_v34[c].astype("category")

    val_fe_v14 = fe_v14[fe_v14["season"] == 2024].copy()
    val_fe_v34 = fe_v34[fe_v34["season"] == 2024].copy()
    ...
    res_v34 = run_one("WITH (v34 초면맞대결 1개 추가, 102개 피처)", fe_v34, val_fe_v34, ALL_FEATURES_V34)
    results.append(res_v34)

    # 5. 요약
    diff_A = res_v34["score_A_holdout"] - res_v14["score_A_holdout"]
    diff_B = res_v34["score_B_oof"] - res_v14["score_B_oof"]

    print("\n" + "=" * 70)
    print("최종 결과 요약 (First-Time Matchup Binary Indicator)")
    print("=" * 70)
    print(f"{'항목':<35} {'v14(WITHOUT)':>12} {'v34(WITH)':>12} {'diff':>10}")
    print(f"{'피처 수':<35} {res_v14['n_features']:>12} {res_v34['n_features']:>12}")
    print(f"{'LGB solo':<35} {res_v14['lgb_solo']:>12.2f} {res_v34['lgb_solo']:>12.2f}")
    print(f"{'CB solo':<35} {res_v14['cb_solo']:>12.2f} {res_v34['cb_solo']:>12.2f}")
    print(f"{'LGB coef':<35} {res_v14['w_lgb']:>12.4f} {res_v34['w_lgb']:>12.4f}")
    print(f"{'CB coef':<35} {res_v14['w_cb']:>12.4f} {res_v34['w_cb']:>12.4f}")
    print(f"{'(A) 홀드아웃':<35} {res_v14['score_A_holdout']:>12.2f} {res_v34['score_A_holdout']:>12.2f} {diff_A:>+10.2f}")
    print(f"{'(B) OOF score':<35} {res_v14['score_B_oof']:>12.2f} {res_v34['score_B_oof']:>12.2f} {diff_B:>+10.2f}")
    print(f"{'가드레일 상태':<35} {res_v14['guardrail_status']:>12} {res_v34['guardrail_status']:>12}")
    print("=" * 70)

    out = "docs/v34_first_matchup_results.csv"
