"""
v15 후보 실험 1: 당해 시즌 폼 기반 매치업 격차 및 상황 상호작용 피처
======================================================================

배경 및 가설:
  - v14에서 Season Decomposition(당해 시즌 폼 분해)이 도입되어
    투수와 타자 각각의 당해 시즌 베이지안 사후확률(current_posterior) 및
    커리어 대비 폼 괴리도(current_history_gap)가 성공적으로 추출됨 (Dacon 976.51 달성).
  - 기존 v10의 matchup_success_diff는 '커리어 전체 누적' 기준의 매치업이었음.
  - 가설:
    1) 당해 시즌 폼 기준 매치업:
       - matchup_current_posterior_diff = season_pitcher_current_posterior - season_batter_current_posterior
       - matchup_current_gap_diff = season_pitcher_current_history_gap - season_batter_current_history_gap
    2) 고레버리지 / 2스트라이크 / 풀카운트 상황에서 당해 폼과의 상호작용:
       - pitcher_form_li = season_pitcher_current_history_gap * li
       - pitcher_current_posterior_li = season_pitcher_current_posterior * li
       - pitcher_form_two_strike = season_pitcher_current_history_gap * is_two_strike
       - pitcher_form_full_count = season_pitcher_current_history_gap * is_full_count
       - batter_form_li = season_batter_current_history_gap * li
...
def add_season_interactions(df):
    """v15 후보: 당해 시즌 폼 기반 매치업 및 레버리지 상호작용 피처 추가"""
    df = df.copy()

    # 1. 당해 시즌 폼 기반 매치업 격차
    df["matchup_current_posterior_diff"] = (
        df["season_pitcher_current_posterior"] - df["season_batter_current_posterior"]
    )
    df["matchup_current_gap_diff"] = (
        df["season_pitcher_current_history_gap"] - df["season_batter_current_history_gap"]
    )

    # 2. 당해 폼과 경기 레버리지/카운트 상호작용
    df["pitcher_form_li"] = df["season_pitcher_current_history_gap"] * df["li"]
    df["pitcher_current_posterior_li"] = df["season_pitcher_current_posterior"] * df["li"]
    df["pitcher_form_two_strike"] = df["season_pitcher_current_history_gap"] * df["is_two_strike"]
    df["pitcher_form_full_count"] = df["season_pitcher_current_history_gap"] * df["is_full_count"]
    df["batter_form_li"] = df["season_batter_current_history_gap"] * df["li"]
...
def main():
    print("=" * 70)
    print("v15 후보 실험: 당해 시즌 폼 기반 매치업 & 레버리지 상호작용 피처")
    ...
    # ── 기본 피처 및 Season Decomposition 생성 (v14 기준) ──
    print("\nv14 기준 피처 생성 중...")
    t0 = time.time()
    ...
    # ── v15 WITH 피처: 매치업 & 레버리지 상호작용 추가 ──
    print("v15 상호작용 피처 추가 중...")
    fe_v15 = add_season_interactions(fe_v14)
    ...
    NEW_COLS_V15 = [c for c in fe_v15.columns if c not in train_full.columns]
    ALL_FEATURES_V15 = CAT_COLS + [c for c in FEATURES_BASE if c not in CAT_COLS] + NEW_COLS_V15

    for c in CAT_COLS:
        fe_v14[c] = fe_v14[c].astype("category")
        fe_v15[c] = fe_v15[c].astype("category")

    val_fe_v14 = fe_v14[fe_v14["season"] == 2024].copy()
    val_fe_v15 = fe_v15[fe_v15["season"] == 2024].copy()
    ...
    res_v15 = run_one(
        "WITH (v15 폼 매치업&상호작용 7개 추가, 108개 피처)",
        fe_v15, val_fe_v15, ALL_FEATURES_V15,
    )
    results.append(res_v15)

    # ── 최종 요약 ──
    diff_A = res_v15["score_A_holdout"] - res_v14["score_A_holdout"]
    diff_B = res_v15["score_B_oof"] - res_v14["score_B_oof"]

    print("\n" + "=" * 70)
    print("최종 결과 요약")
    print("=" * 70)
    print(f"{'항목':<35} {'v14(WITHOUT)':>12} {'v15(WITH)':>12} {'diff':>10}")
    print(f"{'피처 수':<35} {res_v14['n_features']:>12} {res_v15['n_features']:>12}")
    print(f"{'LGB solo':<35} {res_v14['lgb_solo']:>12.2f} {res_v15['lgb_solo']:>12.2f}")
    print(f"{'CB solo':<35} {res_v14['cb_solo']:>12.2f} {res_v15['cb_solo']:>12.2f}")
    print(f"{'(A) 홀드아웃':<35} {res_v14['score_A_holdout']:>12.2f} {res_v15['score_A_holdout']:>12.2f} {diff_A:>+10.2f}")
    print(f"{'(B) OOF score':<35} {res_v14['score_B_oof']:>12.2f} {res_v15['score_B_oof']:>12.2f} {diff_B:>+10.2f}")
    ...
    out = "docs/v15_matchup_interactions_results.csv"
