"""
v18 후보 실험: Multi-Teacher Soft Label Consensus Distillation (소프트 증류)
=============================================================================

배경 및 가설 (Phase2 R35 챔피언 핵심 메커니즘):
  - Brier Score는 (p - y)^2 형태의 2차 손실 함수로, 본질적으로 ~52% 확률인 투구 데이터에서
    극단적인 {0, 1} 하드 레이블은 트리 리프의 과적합과 높은 그래디언트 분산을 유발함.
  - Phase2 R35의 Soft Distillation:
    1) 5-Fold OOF에서 LightGBM과 CatBoost 교사의 앙상블 확률 p_teacher를 생성.
    2) 부드러운 소프트 타겟 y_soft = (1 - w_soft) * y + w_soft * p_teacher (w_soft=0.30) 생성.
    3) Student LightGBM을 y_soft 타겟으로 학습하여 과적합 없이 더 매끄럽고 캘리브레이션된 예측을 유도.
    4) Student 예측을 메타러너 스태킹에 결합.
...
def main():
    print("=" * 70)
    print("v18 후보 실험: Multi-Teacher Soft Label Consensus Distillation")
    print("기준선: v14 (홀드아웃 836.35, OOF 2009.23, Dacon 976.51)")
    print("=" * 70)
    t_start = time.time()

    # 1. 데이터 로드 및 피처 생성
    print("\n[1/5] 데이터 로드 및 v14 피처 생성...")
    ...
    p_teacher_val = 0.5 * (p_lgb_val + p_cb_val)
    print(f"  교사 훈련 완료 :: {time.time()-t0:.1f}s | LGB={brier_skill_score(y_val, p_lgb_val):.2f}, CB={brier_skill_score(y_val, p_cb_val):.2f}")

    # 3. 5-Fold OOF로 교사 합의 예측 생성
    print("\n[3/5] 5-Fold OOF로 교사 소프트 타겟 생성...")
    ...
    # 기준선 (v14 2-way 스태킹)
    stack_v14 = LogisticRegression(random_state=42, max_iter=1000)
    stack_v14.fit(np.column_stack([lgb_oof, cb_oof]), y_train)
    score_v14_val = brier_skill_score(y_val, stack_v14.predict_proba(np.column_stack([p_lgb_val, p_cb_val]))[:, 1])
    score_v14_oof = brier_skill_score(y_train, stack_v14.predict_proba(np.column_stack([lgb_oof, cb_oof]))[:, 1])
    print(f"  v14 기준선: (A) 홀드아웃={score_v14_val:.2f}, (B) OOF={score_v14_oof:.2f}")

    # 4. 소프트 가중치별 Student LightGBM 학습 및 평가
    print("\n[4/5] Soft Weight (w_soft) 스윕 및 Student 증류 모델 평가...")
    W_SOFT_CANDIDATES = [0.15, 0.30, 0.45]
    results = []

    for w_soft in W_SOFT_CANDIDATES:
        t0 = time.time()
        y_soft_train = (1.0 - w_soft) * y_train + w_soft * p_teacher_oof
        
        # Student LightGBM 학습 (cross-entropy with float targets [0, 1])
        ...
        score_student_solo = brier_skill_score(y_val, p_student_val)

        # 5-fold OOF for student
        ...
        # Student + CatBoost Stacking (Student가 원래 LGB를 대체)
        stack_student = LogisticRegression(random_state=42, max_iter=1000)
        stack_student.fit(np.column_stack([student_oof, cb_oof]), y_train)
        preds_st_val = stack_student.predict_proba(np.column_stack([p_student_val, p_cb_val]))[:, 1]
        preds_st_oof = stack_student.predict_proba(np.column_stack([student_oof, cb_oof]))[:, 1]

        score_st_val = brier_skill_score(y_val, preds_st_val)
        score_st_oof = brier_skill_score(y_train, preds_st_oof)
        elapsed = time.time() - t0

        print(f"  w_soft={w_soft:.2f} | Student solo={score_student_solo:.2f} | (A)Holdout(Stack)={score_st_val:.2f} | (B)OOF={score_st_oof:.2f} | {elapsed/60:.1f}분")
        results.append({
            "w_soft": w_soft,
            "student_solo": score_student_solo,
            "holdout_stack": score_st_val,
            "oof_stack": score_st_oof,
            "diff_holdout": score_st_val - score_v14_val,
            "diff_oof": score_st_oof - score_v14_oof,
        })

    # 5. 최종 요약
    print("\n" + "=" * 70)
    print("최종 결과 요약 (Soft Label Distillation)")
    print("=" * 70)
    print(f"{'w_soft':<10} {'Student solo':>15} {'(A) 홀드아웃(Stack)':>20} {'(B) OOF score':>15} {'diff (A)':>10}")
    print(f"{'0.00(v14)':<10} {brier_skill_score(y_val, p_lgb_val):>15.2f} {score_v14_val:>20.2f} {score_v14_oof:>15.2f} {'+0.00':>10}")
    for r in results:
        print(f"{r['w_soft']:<10.2f} {r['student_solo']:>15.2f} {r['holdout_stack']:>20.2f} {r['oof_stack']:>15.2f} {r['diff_holdout']:>+10.2f}")

    best_r = max(results, key=lambda x: x["holdout_stack"])
    print(f"\n최적 w_soft: {best_r['w_soft']} (홀드아웃 diff={best_r['diff_holdout']:+.2f}, OOF diff={best_r['diff_oof']:+.2f})")
    print(f"총 소요 시간: {(time.time()-t_start)/60:.1f}분")
    print("=" * 70)

    out = "docs/v18_soft_distillation_results.csv"
