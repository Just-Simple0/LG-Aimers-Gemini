"""
Autonomous Continuous Research Runner - Wave 4 (v51 ~ v54)
===========================================================

차세대 심층 연구 4대 후보:
  1. [Exp 1] v51: Pitchmix Repertoire Shannon Entropy (구종 다변성 섀넌 엔트로피)
  2. [Exp 2] v52: Count-Dependent Trackman Break Volatility Interaction (카운트별 궤적 변동성 상호작용)
  3. [Exp 3] v53: Home/Away Win Expectancy Leverage Momentum (승리 기대 확률 모멘텀)
  4. [Exp 4] v54: 7-Fold CV Stacking Meta-Learner (7-Fold CV 황금비 스태킹)
...
def main():
    print("=" * 80)
    print("🚀 Autonomous Continuous Runner - Wave 4 (v51 ~ v54)")
    ...
    # =========================================================================
    # [Exp 1] v51: Pitchmix Repertoire Shannon Entropy
    # =========================================================================
    print("\n" + "=" * 70)
    print("▶ [Exp 1/4] v51: Pitchmix Repertoire Shannon Entropy")
    print("=" * 70)
    fe_v51 = fe_v14.copy()
    fb = np.clip(fe_v51["asof_pitcher_fastball_rate"].fillna(0), 1e-5, 1.0)
    br = np.clip(fe_v51["asof_pitcher_breaking_rate"].fillna(0), 1e-5, 1.0)
    os_ = np.clip(fe_v51["asof_pitcher_offspeed_rate"].fillna(0), 1e-5, 1.0)
    tot = fb + br + os_
    p_fb, p_br, p_os = fb / tot, br / tot, os_ / tot
    fe_v51["pitchmix_entropy"] = -(p_fb * np.log(p_fb) + p_br * np.log(p_br) + p_os * np.log(p_os))
    ALL_FEATURES_V51 = ALL_FEATURES_V14 + ["pitchmix_entropy"]
    val_fe_v51 = fe_v51[fe_v51["season"] == 2024].copy()

    res_v51 = run_pipeline(fe_v51, val_fe_v51, ALL_FEATURES_V51, n_splits=5)
    res_v51["experiment"] = "Exp 1 (v51-pitchmix-entropy)"
    batch_results.append(res_v51)
    print(f"  Holdout={res_v51['holdout']:.2f}({res_v51['diff_A']:>+5.2f}) | OOF={res_v51['oof']:.2f}({res_v51['diff_B']:>+5.2f}) | LGB={res_v51['w_lgb']:.2f}, CB={res_v51['w_cb']:.2f} | {res_v51['guardrail_status']}")

    # =========================================================================
    # [Exp 2] v52: 7-Fold CV Stacking Meta-Learner (85.7% Train Per Fold)
    # =========================================================================
    print("\n" + "=" * 70)
    print("▶ [Exp 2/4] v52: 7-Fold CV Stacking Meta-Learner")
    print("=" * 70)
    res_v52 = run_pipeline(fe_v14, val_fe_v14, ALL_FEATURES_V14, n_splits=7)
    res_v52["experiment"] = "Exp 2 (v52-7fold-stacking)"
    batch_results.append(res_v52)
    print(f"  Holdout={res_v52['holdout']:.2f}({res_v52['diff_A']:>+5.2f}) | OOF={res_v52['oof']:.2f}({res_v52['diff_B']:>+5.2f}) | LGB={res_v52['w_lgb']:.2f}, CB={res_v52['w_cb']:.2f} | {res_v52['guardrail_status']}")

    # =========================================================================
    # 종합 요약
    # =========================================================================
    print("\n" + "=" * 90)
    print("🏁 Wave 4 Autoresearch 종합 결과 및 5대 가드레일 판정")
    print("=" * 90)
    ...
    out = "docs/v51_to_v55_batch_results.csv"
