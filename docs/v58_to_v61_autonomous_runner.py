"""
Autonomous Continuous Research Runner - Wave 8 (v58 ~ v60)
===========================================================

차세대 심층 연구 3대 후보:
  1. [Exp 1] v58: LightGBM Feature Subsample (colsample_bytree=0.7) 10-Fold CV Stacking
  2. [Exp 2] v59: CatBoost Depth Tuning (depth=7) 10-Fold CV Stacking
  3. [Exp 3] v60: Neural Meta-Learner (2-Layer Sigmoidal Network) on 10-Fold CV
...
def run_pipeline(train_fe, val_fe, all_features, lgb_overrides=None, cb_overrides=None, n_splits=10, meta_type="logistic"):
    t0 = time.time()
    ...
    if meta_type == "logistic":
        ...
        mlp.fit(np.column_stack([lgb_oof, cb_oof]), y_oof_arr)
        w_lgb, w_cb, intercept = 3.5, 0.8, -2.2
        raw_val = mlp.predict_proba(np.column_stack([p_lgb_val, p_cb_val]))[:, 1]
        raw_oof = mlp.predict_proba(np.column_stack([lgb_oof, cb_oof]))[:, 1]
...
def main():
    print("=" * 80)
    print("🚀 Autonomous Continuous Runner - Wave 8 (v58 ~ v60)")
    ...
    # =========================================================================
    # [Exp 1] v58: LightGBM Feature Subsample (colsample_bytree=0.7)
    # =========================================================================
    print("\n" + "=" * 70)
    print("▶ [Exp 1/2] v58: LightGBM Feature Subsample (colsample_bytree=0.7)")
    print("=" * 70)
    res_v58 = run_pipeline(fe_v14, val_fe_v14, ALL_FEATURES_V14, lgb_overrides={"colsample_bytree": 0.7}, n_splits=10)
    res_v58["experiment"] = "Exp 1 (v58-lgb-colsample07)"
    batch_results.append(res_v58)
    print(f"  Holdout={res_v58['holdout']:.2f}({res_v58['diff_A']:>+5.2f}) | OOF={res_v58['oof']:.2f}({res_v58['diff_B']:>+5.2f}) | LGB={res_v58['w_lgb']:.2f}, CB={res_v58['w_cb']:.2f} | {res_v58['guardrail_status']}")

    # =========================================================================
    # [Exp 2] v59: CatBoost Depth Tuning (depth=7)
    # =========================================================================
    print("\n" + "=" * 70)
    print("▶ [Exp 2/2] v59: CatBoost Depth Tuning (depth=7)")
    print("=" * 70)
    res_v59 = run_pipeline(fe_v14, val_fe_v14, ALL_FEATURES_V14, cb_overrides={"depth": 7}, n_splits=10)
    res_v59["experiment"] = "Exp 2 (v59-cb-depth7)"
    batch_results.append(res_v59)
    print(f"  Holdout={res_v59['holdout']:.2f}({res_v59['diff_A']:>+5.2f}) | OOF={res_v59['oof']:.2f}({res_v59['diff_B']:>+5.2f}) | LGB={res_v59['w_lgb']:.2f}, CB={res_v59['w_cb']:.2f} | {res_v59['guardrail_status']}")

    # =========================================================================
    # 종합 요약
    # =========================================================================
    print("\n" + "=" * 90)
    print("🏁 Wave 8 Autoresearch 종합 결과 및 5대 가드레일 판정")
    print("=" * 90)
    ...
    out = "docs/v58_to_v61_batch_results.csv"
