"""
v39 후보 실험: 세이버메트릭스 경기 국면(TTO) 및 세트포지션(Stretch) 제구 상호작용
(Game Phase & Stretch Pitching Penalty Features)
=====================================================================================

배경 및 야구 도메인 이론:
  - 타순 일주(Times Through the Order) 효과:
    1) 초반(1~3회, phase=0): 1번째 타순, 와인드업, 체력 100% (제구 안정)
    2) 중반(4~6회, phase=1): 2번째 타순, 타자 눈 익음 및 피로 누적
    3) 후반(7회+, phase=2): 불펜 투입 및 경기 후반 레버리지 집중
  - 세트포지션(Stretch) 제구 페널티:
    - 주자가 없을 때(base_state == '000')는 풀 와인드업 투구
    - 주자가 출루했을 때(runner_on_base=1)는 퀵모션/세트포지션 투구로 제구 편차 발생
    - stretch_matchup_interaction = runner_on_base * matchup_success_diff
  - 가설:
    경기 국면과 주자 유무에 따른 투구 메커니즘 변화를 2개 피처로 정제 공급하면
    이중 게이트(A/B) 및 5대 하드 가드레일을 통과할 수 있을 것임.
...
def run_one(label, train_fe, val_fe, all_features):
    ...
    lgb_oof = np.zeros(len(y_oof))
    cb_oof = np.zeros(len(y_oof))
...
def main():
    print("=" * 70)
    print("v39 후보 실험: Game Phase & Stretch Pitching Penalty Features")
    ...
    # 2. v39 국면 & 세트포지션 피처 2개 생성
    fe_v39 = fe_v14.copy()
    runner_on = (fe_v39["base_state"] != "000").astype(int)
    fe_v39["runner_on_base"] = runner_on
    fe_v39["stretch_matchup_diff"] = runner_on * fe_v39["matchup_success_diff"]
    ...
    NEW_COLS_V39 = [c for c in fe_v39.columns if c not in train_full.columns]
    ALL_FEATURES_V39 = CAT_COLS + [c for c in FEATURES_BASE if c not in CAT_COLS] + NEW_COLS_V39

    for c in CAT_COLS:
        fe_v14[c] = fe_v14[c].astype("category")
        fe_v39[c] = fe_v39[c].astype("category")

    val_fe_v14 = fe_v14[fe_v14["season"] == 2024].copy()
    val_fe_v39 = fe_v39[fe_v39["season"] == 2024].copy()
    ...
    res_v39 = run_one("WITH (v39 세트포지션 2개 추가, 103개 피처)", fe_v39, val_fe_v39, ALL_FEATURES_V39)
    results.append(res_v39)

    diff_A = res_v39["score_A_holdout"] - res_v14["score_A_holdout"]
    diff_B = res_v39["score_B_oof"] - res_v14["score_B_oof"]

    print("\n" + "=" * 70)
    print("최종 결과 요약 (Game Phase & Stretch Pitching Penalty)")
    print("=" * 70)
    print(f"{'항목':<35} {'v14(WITHOUT)':>12} {'v39(WITH)':>12} {'diff':>10}")
    print(f"{'피처 수':<35} {res_v14['n_features']:>12} {res_v39['n_features']:>12}")
    print(f"{'LGB solo':<35} {res_v14['lgb_solo']:>12.2f} {res_v39['lgb_solo']:>12.2f}")
    print(f"{'CB solo':<35} {res_v14['cb_solo']:>12.2f} {res_v39['cb_solo']:>12.2f}")
    print(f"{'LGB coef':<35} {res_v14['w_lgb']:>12.4f} {res_v39['w_lgb']:>12.4f}")
    print(f"{'CB coef':<35} {res_v14['w_cb']:>12.4f} {res_v39['w_cb']:>12.4f}")
    print(f"{'(A) 홀드아웃':<35} {res_v14['score_A_holdout']:>12.2f} {res_v39['score_A_holdout']:>12.2f} {diff_A:>+10.2f}")
    print(f"{'(B) OOF score':<35} {res_v14['score_B_oof']:>12.2f} {res_v39['score_B_oof']:>12.2f} {diff_B:>+10.2f}")
    print(f"{'가드레일 상태':<35} {res_v14['guardrail_status']:>12} {res_v39['guardrail_status']:>12}")
    print("=" * 70)

    out = "docs/v39_stretch_penalty_results.csv"
