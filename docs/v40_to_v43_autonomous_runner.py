"""
Autonomous Continuous Research Runner (v40 ~ v43)
==================================================

4대 심층 연구 주제 연속 자율 실행:
  1. [Exp 1] v40: 5-Seed Bagged Models with Bounded SLSQP Meta-Learner
  2. [Exp 2] v41: Leverage-Conditioned Clutch Control Rate Bayesian Gap
  3. [Exp 3] v42: Markov Count State Transition Prior
  4. [Exp 4] v43: Dense-Zone Isotonic Spline Local Calibration
...
def run_standard_pipeline(train_fe, val_fe, all_features, meta_type="logistic"):
    t0 = time.time()
    ...
    if meta_type == "logistic":
        ...
        iso = IsotonicRegression(out_of_bounds="clip", y_min=0.325, y_max=0.755)
        iso.fit(raw_oof, y_oof_arr)
        preds_val = iso.predict(raw_val)
        preds_oof = iso.predict(raw_oof)
...
def main():
    print("=" * 80)
    print("🚀 Autonomous Continuous Runner (v40 ~ v43)")
    ...
    # =========================================================================
    # [Exp 1] v40: Bounded SLSQP Meta-Learner on v14
    # =========================================================================
    print("\n" + "=" * 70)
    print("▶ [Exp 1/4] v40: Bounded SLSQP Meta-Learner (w_lgb in [2.0, 3.8], w_cb in [0.5, 2.0])")
    print("=" * 70)
    res_v40 = run_standard_pipeline(fe_v14, val_fe_v14, ALL_FEATURES_V14, meta_type="bounded_slsqp")
    res_v40["experiment"] = "Exp 1 (v40-bounded-slsqp)"
    batch_results.append(res_v40)
    print(f"  Holdout={res_v40['holdout']:.2f}({res_v40['diff_A']:>+5.2f}) | OOF={res_v40['oof']:.2f}({res_v40['diff_B']:>+5.2f}) | LGB={res_v40['w_lgb']:.2f}, CB={res_v40['w_cb']:.2f} | {res_v40['guardrail_status']}")

    # =========================================================================
    # [Exp 2] v41: Leverage-Conditioned Clutch Control Rate Bayesian Gap
    # =========================================================================
    print("\n" + "=" * 70)
    print("▶ [Exp 2/4] v41: Leverage-Conditioned Clutch Control Rate Gap")
    print("=" * 70)
    fe_v41 = fe_v14.copy()
    # 투수 위기 상황(li >= 1.5) 제구 상호작용
    fe_v41["pitcher_clutch_interaction"] = fe_v41["is_high_leverage"] * fe_v41["pitcher_success_rate_smooth"]
    fe_v41["batter_clutch_interaction"] = fe_v41["is_high_leverage"] * fe_v41["batter_success_rate_smooth"]
    ALL_FEATURES_V41 = ALL_FEATURES_V14 + ["pitcher_clutch_interaction", "batter_clutch_interaction"]
    val_fe_v41 = fe_v41[fe_v41["season"] == 2024].copy()

    res_v41 = run_standard_pipeline(fe_v41, val_fe_v41, ALL_FEATURES_V41, meta_type="logistic")
    res_v41["experiment"] = "Exp 2 (v41-leverage-clutch-decomp)"
    batch_results.append(res_v41)
    print(f"  Holdout={res_v41['holdout']:.2f}({res_v41['diff_A']:>+5.2f}) | OOF={res_v41['oof']:.2f}({res_v41['diff_B']:>+5.2f}) | LGB={res_v41['w_lgb']:.2f}, CB={res_v41['w_cb']:.2f} | {res_v41['guardrail_status']}")

    # =========================================================================
    # [Exp 3] v42: Markov Count State Transition Expectancy Prior
    # =========================================================================
    print("\n" + "=" * 70)
    print("▶ [Exp 3/4] v42: Markov Count Transition Expectancy Prior")
    print("=" * 70)
    fe_v42 = fe_v14.copy()
    # 카운트 불리 상황에서의 기대 회복력: count_diff와 투수 제구율의 곱
    fe_v42["count_recovery_index"] = fe_v42["count_diff"] * fe_v42["pitcher_success_rate_smooth"]
    ALL_FEATURES_V42 = ALL_FEATURES_V14 + ["count_recovery_index"]
    val_fe_v42 = fe_v42[fe_v42["season"] == 2024].copy()

    res_v42 = run_standard_pipeline(fe_v42, val_fe_v42, ALL_FEATURES_V42, meta_type="logistic")
    res_v42["experiment"] = "Exp 3 (v42-count-transition-prior)"
    batch_results.append(res_v42)
    print(f"  Holdout={res_v42['holdout']:.2f}({res_v42['diff_A']:>+5.2f}) | OOF={res_v42['oof']:.2f}({res_v42['diff_B']:>+5.2f}) | LGB={res_v42['w_lgb']:.2f}, CB={res_v42['w_cb']:.2f} | {res_v42['guardrail_status']}")

    # =========================================================================
    # [Exp 4] v43: Dense-Zone Isotonic Spline Local Calibration
    # =========================================================================
    print("\n" + "=" * 70)
    print("▶ [Exp 4/4] v43: Dense-Zone Isotonic Spline Local Calibration on OOF")
    print("=" * 70)
    res_v43 = run_standard_pipeline(fe_v14, val_fe_v14, ALL_FEATURES_V14, meta_type="isotonic_local")
    res_v43["experiment"] = "Exp 4 (v43-spline-local-calibration)"
    batch_results.append(res_v43)
    print(f"  Holdout={res_v43['holdout']:.2f}({res_v43['diff_A']:>+5.2f}) | OOF={res_v43['oof']:.2f}({res_v43['diff_B']:>+5.2f}) | LGB={res_v43['w_lgb']:.2f}, CB={res_v43['w_cb']:.2f} | {res_v43['guardrail_status']}")

    # =========================================================================
    # 종합 요약
    # =========================================================================
    print("\n" + "=" * 90)
    print("🏁 4-Batch Autoresearch (v40 ~ v43) 종합 결과 및 5대 가드레일 판정")
    print("=" * 90)
    ...
    out = "docs/v40_to_v43_batch_results.csv"
