"""
v50 실험: 10-Fold CV 고밀도 스태킹 + 안전 확률 포락선 (Safe 10-Fold Stacking)
=============================================================================

배경 및 가설:
  - v46(10-Fold CV 스태킹)에서 90% 훈련 데이터 기반 고밀도 OOF를 사용한 결과:
    (A) 홀드아웃: 836.35 -> 839.58 (+3.23 상승)
    (B) OOF score: 2009.23 -> 2015.60 (+6.37 상승)
    메타러너 계수: LGB=3.88, CB=0.63 (양수 균형 완벽)
  - 단, 예측 최솟값이 0.3110으로 하한선(0.320)을 미세 이탈하여 가드레일에 걸림.
  - 가설:
    10-Fold CV 스태킹의 강력한 일반화 성능을 유지하면서,
    최종 예측 확률을 [0.325, 0.755]로 안전하게 클리핑하면
    이중 게이트 동반 상승과 5대 하드 가드레일 100% 합격을 동시에 달성할 수 있음!
...
def main():
    print("=" * 80)
    print("v50 실험: 10-Fold CV 고밀도 스태킹 + 안전 확률 포락선 (Safe 10-Fold Stacking)")
    ...
    print("\n[Step 1] 베이스 모델 학습 (LightGBM 171 iters, CatBoost 360 iters)...")
    ...
    print("\n[Step 2] 10-Fold CV 고밀도 OOF 생성 (Fold당 90% 데이터 훈련)...")
    ...
    print("\n[Step 3] 로지스틱 회귀 메타러너 학습 및 가중치 검수...")
    ...
    # Step 4: 안전 확률 포락선 [0.325, 0.755] 적용
    ...
    print("\n" + "=" * 80)
    print("최종 10-Fold CV 고밀도 안전 스태킹 결과")
    print("=" * 80)
    ...
    print(f"  예측 범위    : [{np.min(preds_val_safe):.4f}, {np.max(preds_val_safe):.4f}], Std: {np.std(preds_val_safe):.4f}")
    print(f"  가드레일 상태: {guardrail_status} {guardrail_msg}")
    print(f"  총 소요 시간  : {(time.time()-t0)/60:.1f}분")
    print("=" * 80)

    out = "docs/v50_safe_10fold_results.csv"
    res_df = pd.DataFrame([{
        "holdout": score_val,
        "oof": score_oof,
        "diff_A": diff_A,
