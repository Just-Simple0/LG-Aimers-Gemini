"""
v38 후보 실험: 5대 가드레일 제약 하의 안전한 온도 조절 로짓 볼록 블렌딩
(Safe Calibrated Convex Blend under 5-Point Hard Guardrails)
=============================================================================

배경 및 가설:
  - v37에서 (A) 홀드아웃 846.10 (+9.75), (B) OOF score 2010.56 (+1.33)으로
    이중 게이트가 모두 개선되었으나, T=0.9195로 인해 min=0.3102가 되어 가드레일에 걸림.
  - 가설:
    1) 온도 T를 T >= 1.00 (보수적 볼록 수축: T in [1.00, 1.20])으로 제약
    2) 가중치 w_lgb in [0.20, 0.80] 균형 유지
    3) 최종 예측 확률을 [0.325, 0.755] 안전 구간으로 완벽하게 클리핑
    - 이중 게이트 (A) > 840, (B) > 2009.5 동반 달성 및 5대 하드 가드레일 100% 합격 검증!
...
def main():
    print("=" * 80)
    print("v38 후보 실험: 5대 가드레일 제약 하의 안전한 온도 조절 로짓 볼록 블렌딩")
    ...
    print("\n[Step 2] 안전한 온도 스케일링 볼록 파라미터 (T >= 1.00, w in [0.2, 0.8]) 최적화...")
    eps = 1e-6

    def calibrate_fn_safe(p_l, p_c, w, T, b):
        p_blend = np.clip(w * p_l + (1.0 - w) * p_c, eps, 1.0 - eps)
        scaled_logit = (logit(p_blend) + b) / T
        p_raw = expit(np.clip(scaled_logit, -10, 10))
        return np.clip(p_raw, 0.325, 0.755)

    def loss_func(params):
        w, T, b = params
        p_cal = calibrate_fn_safe(lgb_oof, cb_oof, w, T, b)
        return np.mean((p_cal - y_oof_arr) ** 2)

    # Hard safe bounds: w in [0.20, 0.80], T in [1.00, 1.25], b in [-0.05, 0.05]
    init_params = [0.50, 1.00, 0.0]
    bounds = [(0.20, 0.80), (1.00, 1.25), (-0.05, 0.05)]
    opt_res = minimize(loss_func, init_params, method="L-BFGS-B", bounds=bounds)

    opt_w, opt_T, opt_b = opt_res.x
    print(f"  최적 안전 파라미터: w_lgb={opt_w:.4f}, w_cb={1.0-opt_w:.4f}, T={opt_T:.4f}, b={opt_b:.4f}")

    # 평가
    preds_val_cal = calibrate_fn_safe(p_lgb_val, p_cb_val, opt_w, opt_T, opt_b)
    preds_oof_cal = calibrate_fn_safe(lgb_oof, cb_oof, opt_w, opt_T, opt_b)
    ...
    print("\n" + "=" * 80)
    print("최종 안전한 온도 조절 로짓 볼록 블렌딩 평가 결과")
    print("=" * 80)
    ...
    out = "docs/v38_safe_calibrated_blend_results.csv"
    res_df = pd.DataFrame([{
        "opt_w": opt_w,
        "opt_T": opt_T,
        "opt_b": opt_b,
