"""
v24 최종 전체 데이터 재학습 및 제출 패키징 스크립트 (Career Smooth k=50)
========================================================================

1. 2019~2024 전체 학습 데이터(1,475,092행)로 SeasonDecompositionEncoder fit
2. Career Smoothing k=50 적용 및 101개 피처 생성
3. 5-fold KFold OOF로 LightGBM (188 iter) + CatBoost (338 iter) 예측 생성
4. LogisticRegression 메타러너 fit
5. 전체 데이터로 최종 LightGBM / CatBoost fit
6. model/ensemble.pkl 저장 (순수 dict 직렬화)
7. submissions/v24_career_smooth.zip 패키징 (24자 <= 30자 규정)
8. 독립 임시 환경에서 5행 스모크 테스트 및 245,789행 대용량 스트레스 테스트
...
def main():
    print("=" * 70)
    print("v24 최종 전체 데이터 재학습 및 패키징 시작 (Career Smooth k=50)")
    print("=" * 70)
    t_start = time.time()
    ...
    print("\n[3/7] Season Decomposition 인코더 fit 및 훈련 피처 생성 (k=50)...")
    t0 = time.time()
    ...
    for fold_i, (tr_idx, oof_idx) in enumerate(kf.split(X_train_df)):
        ...
        print(f"    KFold {fold_i+1}/5 :: {time.time()-t_fold:.1f}s")
    ...
    ZIP_PATH = "submissions/v24_career_smooth.zip"
